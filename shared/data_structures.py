...
class Dataset:
    def __init__(self, json_file, pred_file=None, doc_range=None, entity_type='ner'): # added entity_type here
...
class Document:
    def __init__(self, js, entity_type="ner"):
        self._doc_key = js["doc_key"]
        # REVIEW: why is "predicted_relations not ignored here?"
        entries = fields_to_batches(js, keys_to_ignore=["doc_key", "clusters", "predicted_clusters", "section_starts"])
        ...
        self.sentences = [Sentence(entry, sentence_start, sentence_ix, entity_type=entity_type)
                          for sentence_ix, (entry, sentence_start)
                          in enumerate(zip(entries, sentence_starts))]#TODO : understand Sentence()
...
class Sentence:
    def __init__(self, entry, sentence_start, sentence_ix, entity_type="ner"):
        ...
        #if "ner" in entry:
        if entity_type in ["ner", "entities_t"]:
            self.ner = [NER(this_ner, self.text, sentence_start)
                        for this_ner in entry[entity_type]]

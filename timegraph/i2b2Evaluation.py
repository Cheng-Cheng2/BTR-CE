...
from temporal_evaluation_adapted import evaluate_two_files, evaluate_two_files_for_subgroup_analysis, evaluate_two_files_for_link_type_based_analysis, evaluate_two_files_for_marker_type_based_analysis
import re
import os
import argparse
...
def get_final_result(totaltlinkScores, entities_to_evaluate='tlink'):
    """
    
    """
    # if entities_to_evaluate in ['tlink', 'all']:
    ...
    print("""                   
            TLINK:
                ...
                F measure : \t"""+'%.4f'%(fScore)+"""
                """+'\n{:*^47}'.format(''))
    return [final_tlink_prec, final_tlink_recall, averagePR, fScore]

def get_scores_per_doc(rtype, total_link_scores_dict, tlinkScores):

    # precLinkCount, recLinkCount, precMatchCount,  recMatchCount = tlinkScores
    
    # note: accumulate to totaltlinkScores
    for i in range(0,4):
        total_link_scores_dict[rtype][i] += tlinkScores[i]
    ...
    print ("""
            ...
            Average P&R : \t\t"""+'%.4f'%(averagePR)+"""
            F measure : \t\t"""+'%.4f'%(fScore) + '\n')
    return total_link_scores_dict

def get_sub_group_analysis_results(goldDir, systemDir, entities_to_evaluate='all', span_match_type='overlap', evaluation_option="tempeval"):
    rel_types = ['ALL','BEFORE', 'AFTER', 'SIMULTANEOUS']
    final_result_dict = {}    
    if os.path.isdir(goldDir) and os.path.isdir(systemDir):
        goldFileList=os.listdir(goldDir)
        systemFileList=os.listdir(systemDir)
        total_link_scores_dict = {rtype:[0,0,0,0] for rtype in rel_types}
        if len(goldFileList)==len(systemFileList):
            ...
            for file in goldFileList:
                ...
                # NOTE: adapt here
                # if entities_to_evaluate in ['tlink','all']:
                #     if evaluation_option=='tempeval':
                tlinkScores=evaluate_two_files_for_subgroup_analysis(os.path.join(goldDir,file),os.path.join(systemDir,file),systemDic,goldDic)
                tlinkScores_sub_dict = {}
                tlinkScores_sub_dict['ALL'] = tlinkScores[:4]
                for rtype in rel_types[1:]:
                    tlinkScores_sub_dict[rtype] = tlinkScores[4][rtype]
                
                for rtype in rel_types:
                    total_link_scores_dict = get_scores_per_doc(rtype, total_link_scores_dict, tlinkScores_sub_dict[rtype])    
                # debug: np.array(total_link_scores_dict['ALL'])
                # agg_subs = np.array([total_link_scores_dict[t] for t in ['BEFORE', 'AFTER', 'SIMULTANEOUS']]).sum(axis=0)
                #print(total_link_scores_dict)
            # NOTE: accumulate to final_tlink
            
            for rel_type in rel_types:
                final_result_dict[rel_type] = get_final_result(total_link_scores_dict[rel_type])       
        
        return final_result_dict
        
        
def get_i2b2_results(goldDir, systemDir, entities_to_evaluate='all', span_match_type='overlap', evaluation_option="tempeval"):
    rel_types = ['ALL']#,'BEFORE', 'AFTER', 'SIMULTANEOUS']
    final_result_dict = {}    
    if os.path.isdir(goldDir) and os.path.isdir(systemDir):
        goldFileList=os.listdir(goldDir)
        ...
        if len(goldFileList)==len(systemFileList):
            ...
            # for rel_type in rel_types:
            #     final_result_dict[rel_type] = get_final_result(total_link_scores_dict[rel_type])       
            final_result_dict = {t:s for t,s in zip(['precision', 'recall', 'pr', 'f1'], get_final_result(total_link_scores_dict['ALL']))}
        return final_result_dict


def get_type_based_results(goldDir, systemDir, rel_types=[], which_type="marker_type", marker_dict=None):
    #rel_types = ['ALL','BEFORE', 'AFTER', 'SIMULTANEOUS']
    #rel_types = ['ALL', 'SEC', 'ET_OTHER', 'ET', 'TT', 'EE']
    rel_types = ['ALL'] + rel_types
    final_result_dict = {}    
    if os.path.isdir(goldDir) and os.path.isdir(systemDir):
        goldFileList=os.listdir(goldDir)
        systemFileList=os.listdir(systemDir)
        ...
        if len(goldFileList)==len(systemFileList):
            ...
            for file in goldFileList:
                ...
                #     if evaluation_option=='tempeval':
                tlinkScores = None
                if which_type=="link_type":
                    tlinkScores=evaluate_two_files_for_link_type_based_analysis(os.path.join(goldDir,file),os.path.join(systemDir,file),systemDic,goldDic)
                elif which_type=="marker_type":
                    doc_id = file.split('.xml')[0]
                    tlinkScores=evaluate_two_files_for_marker_type_based_analysis(os.path.join(goldDir,file),os.path.join(systemDir,file),systemDic,goldDic, doc_id, marker_dict, rel_types)
                tlinkScores_sub_dict = {}
                tlinkScores_sub_dict['ALL'] = tlinkScores[:4]
                for rtype in rel_types[1:]:
                    tlinkScores_sub_dict[rtype] = tlinkScores[4][rtype]
            ...
            for rel_type in rel_types:
                final_result_dict[rel_type] = get_final_result(total_link_scores_dict[rel_type])       
            #final_result_dict = {t:s for t,s in zip(['precision', 'recall', 'pr', 'f1'], total_link_scores_dict['ALL'])}
        return final_result_dict
